...
if ($request->query->get('id') !== null){
    ...
    // #[Route('/filter/{id}', name: 'app_product_filter', methods: ['GET'])]
    // public function filter(int $id, ProductRepository $productRepository, CategoryRepository $categoryRepository): Response
    // {
    //     return $this->render('product/index.html.twig', [
    ...
    public function edit(Request $request, Product $product, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($image != null && $product->getImage() !== null) {
                ...
                if (file_exists($this->getParameter('product_image_directory') . $product->getImage())) {
    ...
    public function delete(Request $request, Product $product, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete' . $product->getId(), $request->getPayload()->get('_token'))) {
            ...
            if ($product->getImage() !== null && file_exists('uploads/image/' . $product->getImage())) {
                unlink('uploads/image/' . $product->getImage());
            }
